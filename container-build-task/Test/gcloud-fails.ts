...
/**
 * @fileoverview This is a test script for the container-build task with
 *   a failure in gcloud.
...
import {failResult} from './test-constants';
...
const fullImageTag = `${registry}/projectId/${imageName}`;
...
const execString = [
  '/mocked/tools/gcloud', 'container builds submit --quiet --format=json',
  '--project=projectId', `--credential-file-override="${jsonKeyFilePath}"`,
  `"${deployPath}"`, `--config="${originConfigPath}"`,
  `--substitutions=_IMAGE="${fullImageTag}"`
].join(' ');

const answers: TaskLibAnswers = getDefaultAnswers();
answers.exec[execString] = failResult;
